...
# Function to get the max, min, and mean from an array parameter 'arr'; prints the values
def get_min_max(arr):
    #arr[arr>1e3] = np.nan
    min_ = np.nanmin(arr)
    avg_ = np.nanmean(arr)
...
# Function to export the water table (wt) 
def export_wt(work_dir, wt_ele, ts):
    gridShp = 'input/shp/grid_274_geo_rc2.shp' # Variable of the shape file to be read in from hardcoded location
    grid = gpd.read_file(gridShp) # Reads the shapefile and ascribes it to variable 'grid'
    dfloc = pd.DataFrame() # New Pandas dataframe variable, followed by assigning 'grid' variable specific columns to 'dfloc'
    dfloc['row'] = grid['row']
    dfloc['column'] = grid['column']
    dfloc['XCoor'] = grid['XCoor']
    dfloc['YCoor'] = grid['YCoor']
    dfloc['IBND'] = grid['IBND']
    dfloc['val'] = np.reshape(wt_ele, [nr*nc, 1]) # Reshaping the array 'wt_ele' (without changing its data). 
    # The newshape parameter of numpy.reshape() function is is ((number of rows x number of columns), 1), so a 1-D array that is nrow x ncol long
    # Output file variable ofile. os.path.join returns concatenation of path "[work_dir]/output/wt_ele_TS{ts}_ver_{today's date}.csv" 
    ofile = os.path.join(work_dir, 'output',
                         f'wt_ele_TS{ts}_ver_{today_dt}.csv')
    dfloc.to_csv(ofile, index=False) #Write the dfloc reshaped variable to the otput file defined in "ofile"
    print(f'Save {ofile}\n')

# Function to export an array to a shapefile
# Three required parameters:
# "arr" - the array to be converted to a shape file
# "gridShp" - model grid shapefile 
# "ofile" - output file
def arr2shp(arr,gridShp,ofile):
    '''
    Export an arry to a shapefile
    '''
    nr, nc = arr.shape # The .shape method returns a tuple representing the dimensions, rows and column, of the data frame variable "arr". Two variables to receive the single values for row and column
    print(nr, nc)
    val = np.reshape(arr, nr*nc) # Assigns the variable "val" to the reshaping of "arr" to the new shape of (nr*nc)

    # use model grid shapefile geometry
    # model grid shapefile
    try:
        #gridShp = os.path.join('input', 'shp_files', 'grid_274_geo_rc.shp')
        gdf = gpd.read_file(gridShp) # Reading in one of the required parameters of "arr2shp", "gridShp"
        #gdf.head(5)
        #gdf.plot()
    except:
    ...
    df['val'] = val # New dataframe variable "df" acquiring 'val' 
...
# Function to read an ucn MODFLOW file.
def read_ucn(ifile_ucn):
    #ucnobj = bf.UcnFile(ifile_ucn, precision='double')
    ucnobj = bf.UcnFile(ifile_ucn, precision='single') # reads in the "ifile_ucn"
    times = ucnobj.get_times() # returns a list of time contained in the binary head file
    data = ucnobj.get_alldata(mflay=None, nodata=-1) # get pathline data from the pathline file for all pathlines and all times
...
def process_riv(ml):
    ...
    for sp in range(len(riv)):
        stage.append(riv[sp]['stage'][0]) # at first cell
...
if __name__ == "__main__":   
    ...
    df_pred = pd.read_csv(f'input/stress_periods_2021-2125.csv')
